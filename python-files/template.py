   ...
   "panels": [
     {
       ...
       "description": "Main Dashboard For ALL SERVERs",
       "flowchartsData": {
         "flowcharts": [
           {
             "allowDrawio": false,
       ...
       "targets": [
         {
           "datasource": "prometheus",
           "editorMode": "code",
           "expr": "(sum by(instance,server)(irate(node_cpu_seconds_total{mode!=\"idle\"}[$__rate_interval])) / on(instance,server) group_left sum by(instance,server) (irate(node_cpu_seconds_total[$__rate_interval]))) * 100",
           "hide": false,
         ...
         },
         {
           "datasource": "prometheus",
           "editorMode": "code",
           "expr": "(topk(1,node_filesystem_size_bytes{fstype!~'tmpfs.*|nfs.*'}) by(device) - node_filesystem_avail_bytes{fstype!~'tmpfs.*|nfs.*|fuse.sshfs'})/node_filesystem_size_bytes *100",
           "hide": false,
           "instant": false,
           "legendFormat": "HDD{{server}}",
           "range": true,
           "refId": "Disk Space Used"
         }
       ],
       "title": "",
       "transparent": true,
